...
const SignUp = () => {
  ...
  const fullname = useRef();
    ...
    const handleSignup = (e) => {
      e.preventDefault();
      if(password.current.value !== retype_password.current.value) retype_password.current.setCustomValidity('password does not match')
      else{
        dispatch(signup({
  ...
  return (
    <div className='h-screen w-screen flex items-center justify-center relative bg-slate-100'>
        <div className='flex flex-col items-center justify-center gap-4 absolute top-6'>
            ...
            <p onClick={()=> navigate('/signin')} className='text-white cursor-pointer   font-medium capitalize hover:bg-blue-700 transition-all duration-200 ease-out text-center rounded shadow-md bg-blue-500 px-3 py-1 flex items-center ' >sign in</p>
        </div>
        <form onSubmit={handleSignup} className=' h-[26rem] w-[26rem] flex flex-col items-center justify-center gap-4 bg-white rounded-md shadow-md'>
            <BsFacebook fontSize={40} className='text-blue-500' />
            <div className=' border-2 py-2  px-4 rounded-md'>
                <input onChange={onchange} ref={username} required  type="text" name='username' className='text-gray-500 w-[250px] bg-transparent border-none outline-none' placeholder='username' />
            </div>
            <div className=' border-2 py-2  px-4 rounded-md'>
                <input onChange={onchange} ref={fullname} required  type="text" name='fullname' className='text-gray-500 w-[250px] bg-transparent border-none outline-none' placeholder='fullname' />
            </div>
            ...
            <div className=' border-2 py-2  px-4 rounded-md'>
                <input onChange={onchange} ref={retype_password} minLength="8" required type="password" name='retype_password' className='text-gray-500 w-[250px] bg-transparent border-none outline-none' placeholder='retype password' />
            </div>
            ...
            <div className='text-white cursor-pointer hover:bg-blue-700 transition-all duration-200 ease-out text-center rounded shadow-md bg-blue-500 px-4 text-sm py-2 flex items-center '>
                ...
                <button  className='  font-medium capitalize' >sign up </button>
            </div>
        </form>
    </div>
  )
